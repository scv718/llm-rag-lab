...
with col_left:
    ...
    with st.expander("📂 프로젝트 문서", expanded=False):
        ...
        if not pid:
            ...
            if up_docs and not st.session_state.get("docs_uploaded"):
                for f in up_docs:
...
with col_mid:
    ...
    if send:
        ...
        if msg:
            ...
            if st.session_state["upload_blobs"]:
                for blob in st.session_state["upload_blobs"]:
